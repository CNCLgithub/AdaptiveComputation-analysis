...
sum_compute_diff <- left_join(good_subject_data, model_data) %>%
  ...
  mutate(scene_sum_comp = sum(compute),
         scene_sum_diff_mu = mean(scene_sum_diff)) %>%
  ungroup() %>%
  distinct()
...
```{r}
# s <- sample_subject_data(subject_norm, 50)
s <- subject_norm
...
n_iters <- 1000
n_subjects <- 10 # sampled subjects with replacement for one bs iteration
...
correlation_data %>%
  ...
  ggtitle("scene-level correlation values \n
          blue line - 95% CI for null, red line - mean h1") +
...
correlation_data %>%
  ggplot() +
  ggtitle("across-scenes correlation values (\"one long vector method\") \n
          blue line - 95% CI for null, red line - mean h1") +
  geom_density(aes(x=h1_full, color="h1")) +
  geom_density(aes(x=null_full, color="null")) +
  geom_vline(aes(xintercept = mean(h1_full)), color="red") +
  geom_vline(aes(xintercept = quantile(null_full, 0.975)), color="blue")

# z_95 = 1.960

cor_data_summary <- correlation_data %>%
  pivot_longer(cols=c(h1_scene, h1_full, null_scene, null_full),
               names_to="hypothesis",
               values_to="correlation") %>%
  mutate(full = grepl("full", hypothesis, fixed=TRUE)) %>%
  group_by(hypothesis) %>%
  summarise(mu = mean(correlation),
            # se = z_95*sd(correlation)/sqrt(n_iters),
            low = quantile(correlation, 0.025),
            high = quantile(correlation, 0.975),
            full = all(full))

cor_data_summary %>%
  filter(full) %>%
  ggplot(aes(hypothesis, mu)) +
  geom_col() +
  ylab("correlation") +
  geom_errorbar(aes(ymin = low, ymax = high), width=0.3) +
  ggtitle("Concatenated vector across scenes correlations)")

cor_data_summary %>%
  filter(!full) %>%
  ggplot(aes(hypothesis, mu)) +
  ggtitle("Scene level correlations)") +
  geom_col() +
  ylab("correlation") +
  geom_errorbar(aes(ymin = low, ymax = high), width=0.3) 
...
#             sd = sd(h1_scene),
#             h1_scene_low = h1_scene_mu + (1.960*sd/sqrt(n_iters)),
#             h1_scene_high = h1_scene_mu - (1.960*sd/sqrt(n_iters))) %>%
#   arrange(h1_scene_mu) %>%
...
```{r split half correlation analysis}
sample_halves <- function(data) {
  IDs <- unique(data$ID)
  sampled_IDs <- sample(IDs, length(IDs) %/% 2) # splitting data in half (integer division)
  
  data %>%
    mutate(first_half = ID %in% sampled_IDs)
}


bootstrap_split_half <- function() {
  h <- sample_halves(subject_norm)
  ...
  a1 <- agg_subject_data(first_half) %>%
    rename(diff_z_1 = diff_z) %>%
    select(scene, frame, diff_z_1)
  
  a2 <- agg_subject_data(second_half) %>%
    rename(diff_z_2 = diff_z) %>%
    select(scene, frame, diff_z_2)

  combined_data <- inner_join(a1, a2, by=c("scene", "frame"))
  scenes <- unique(combined_data$scene)
  
  cor_scene <- combined_data %>%
    group_by(scene) %>%
    summarise(cor_scene = cor(diff_z_1, diff_z_2), .groups="drop_last") %>%
    ungroup() %>%
    pull(cor_scene)
  
  cor_full = cor(combined_data$diff_z_1, combined_data$diff_z_2)
...
for (i in seq(n_iters)) {
  ...
  split_half_data <- split_half_data %>%
    bind_rows(bootstrap_split_half())
}
...
split_half_data %>%
  ggplot() +
  ggtitle("scene-level split half correlation values") +
  geom_density(aes(x=cor_scene, color="split half cor")) +
  geom_vline(xintercept = mean(split_half_data$cor_scene), color="red") +
  geom_vline(xintercept = quantile(split_half_data$cor_scene, 0.025), color="red", linetype="dotted") +
  geom_vline(xintercept = quantile(split_half_data$cor_scene, 0.975), color="red", linetype="dotted")

split_half_data %>%
  ggplot() +
  ggtitle("across-scenes split half correlation values (\"one long vector method\")") +
  geom_density(aes(x=cor_full, color="split half cor")) +
  geom_vline(aes(xintercept = mean(cor_full)), color="red") +
  geom_vline(xintercept = quantile(split_half_data$cor_full, 0.025), color="red", linetype="dotted") +
  geom_vline(xintercept = quantile(split_half_data$cor_full, 0.975), color="red", linetype="dotted")


# cor_data_summary <- split_half %>%
#   pivot_longer(cols=c(h1_scene, h1_full, null_scene, null_full),
#                names_to="hypothesis",
#                values_to="correlation") %>%
#   mutate(full = grepl("full", hypothesis, fixed=TRUE)) %>%
...
split_half_data %>%
  group_by(scene) %>%
  summarise(cor_scene_mu = mean(cor_scene),
            cor_scene_low = quantile(cor_scene, 0.025),
            cor_scene_high = quantile(cor_scene, 0.975)) %>%
  arrange(cor_scene_mu) %>%
  mutate(scene=factor(scene, levels=scene)) %>%
  ggplot(aes(x=scene)) +
  ggtitle("correlations in particular scenes") +
  ylab("correlation") +
  geom_bar(aes(y=cor_scene_mu), stat='identity') +
  geom_errorbar(aes(ymin=cor_scene_low, ymax=cor_scene_high))
