...
prep_ecc_data <- read_csv("output/isr_inertia_480_probe_map_eccentricity.csv") %>%
  filter(scene <= 40)
...
with_probes <- subject_data %>%
  nest_by(ID) %>%
  mutate(full = list(right_join(data, probe_timings))) %>%
  select(-data) %>%
  unnest(cols = full) %>%
...
Looking at various covariates

```{r}
wm_solo <- ate_by_probe %>%
  with(lm(hr ~ zttwm))

tm_solo <- ate_by_probe %>%
  with(lm(hr ~ ztttm))

dm_solo <- ate_by_probe %>%
  with(lm(hr ~ tracker_to_dot_mean))

wm_dm <-  ate_by_probe %>%
  with(lm(hr ~ tracker_to_dot_mean + tracker_to_dot_mean))

```

```{r}
sg <- stargazer::stargazer(ate_by_probe,type = "text")

```

```{r}
sg <- stargazer::stargazer(tttm_ttwm, tttm_hr, res_res_fit,type = "text")
