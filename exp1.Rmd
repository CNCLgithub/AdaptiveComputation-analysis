...
```{r}
full_data %>%
ggplot(aes(x = epoch, y = att, color = factor(tracker_rank))) +
  geom_point() +
...
```{r}
full_data %>%
ggplot(aes(x = epoch, y = pr, color = factor(tracker_rank))) +
  geom_point() +
  facet_wrap(vars(scene)) +
ggtitle("Probe accuracy vs Att within scene")
```
