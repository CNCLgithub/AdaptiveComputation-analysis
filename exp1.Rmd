...
exp_data <- read_csv("data/isr_inertia_extended_probe_map_nd.csv") %>%
  group_by(scene) %>%
...
subject_performance <- subject_data %>%
  ...
  summarise(acc = mean(correct),
            ...
            passed = abs(acc - 0.5) > perm_thresh*se)
...
```{r}
human_pd_by_design <- cate_data %>%
  iv_robust(pr ~ att_scored | zcrowding,  data = .)
summary(human_pd_by_design)
