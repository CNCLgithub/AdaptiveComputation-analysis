...
model_data <- read_csv("performance_compute_target_attention.csv")  %>%
  pivot_longer(cols = starts_with("dot_"),
               names_to = "dot",
               values_to = "model_p_target") %>%
  mutate(dot = str_sub(dot, -1)) %>%
...
full_data %>%
  ggplot(aes(x = model_p_target, y = avg_p_target, color = scene)) +
  geom_point() +
  facet_wrap("dot")
