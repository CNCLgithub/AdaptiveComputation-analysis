...
perf_model %>%
  summary()
good_full_data$td_resid = perf_model$residuals
...
good_full_data %>%
  with(lm(difficulty_mu ~ log(cycles))) %>%
  summary()
...
pred_compute <- good_full_data %>%
  with(lm(log(cycles) ~ td_mu))

pred_difficulty <- good_full_data %>%
  with(lm(difficulty_mu ~ td_mu))

good_full_data <- good_full_data %>%
  mutate(res_difficulty = pred_difficulty$residuals,
         res_compute = pred_compute$residuals)
