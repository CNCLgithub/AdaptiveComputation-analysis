...
# scene, frame
probe_timings <- read_csv("../data/exp2/exp2_probes_hatt_timings.csv") %>%
  filter(scene <= 40) %>%
...
subject_data <- read_csv("../data/exp2/parsed_trials_exp2_hatt_probes.csv") %>%
  select(-c(WID)) %>%
...
```{r}
hits_by_subj_scene <- with_probes %>%
...
fp_by_subj_scene <- with_probes %>%
  ...
  summarise(fp = any(fp)) %>%
  group_by(ID, scene) %>%
  summarise(fpr = sum(fp))

nprobes = nrow(probe_timings)

probe_by_subj <- hits_by_subj_scene %>%
  left_join(fp_by_subj_scene) %>%
  group_by(ID) %>%
  summarise(hr = sum(hits) / nprobes,
            fp = sum(fpr) / nprobes)
...
```{r}
n_subjs = sum(subject_performance$passed, na.rm = TRUE)
...
slim_ate_by_probe <- ate_by_probe %>%
  select(scene, frame, tracker, probe, epoch, hr, cycles_mu, att_smoothed, ttwm, tttm) %>%
  filter(scene == 2) 
...
timing_data <- good_subjects_data %>%
  ...
  nest_by(ID) %>%
  mutate(idata = list(filter(exp_data, scene %in% spec_scene)),
...
timing_data %>%
  ggplot(aes(x = frame)) +
  geom_line(aes(y = att_smoothed, color = factor(tracker))) +
  geom_vline(aes(xintercept = probe, color = "probe_onset"), linetype = "dashed") + 
  # geom_histogram(aes(x =  response_frame), binwidth = 6) +
  facet_grid(rows = vars(scene))
```
...
ate_by_probe %>%
  ggplot(aes(x = total_exp_att, y = hr)) +
  geom_point(aes(color = high_att))
