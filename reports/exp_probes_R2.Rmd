...
model_perf <- read_csv("../data/probes/exp_probes_na_perf.csv") %>%
  group_by(scene) %>%
...
nested_by_att <- ate_by_probe %>%
mutate(att_lvl = cut_number(total_cycles, n = 4)) %>%
  group_by(att_lvl) %>%
  nest()
...
sp_fits <- nested_by_att %>%
  mutate(fit = map(data, ~ lm(hr ~ spatial_importance, data = .x)),
         smry = map(fit, ~ summary(.x)),
         r.squared =  map_dbl(smry, ~ .x[["r.squared"]]),
         p.val = map_dbl(fit,~ glance(.x)[["p.value"]])) %>%
  select(-c(data, fit, smry)) %>%
