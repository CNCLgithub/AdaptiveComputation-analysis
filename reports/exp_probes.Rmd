...
exp_data <- read_csv("../data/probes/probe_covariates.csv")

model_perf <- read_csv("../data/probes/ac_td_perf.csv") %>%
  group_by(scene) %>%
...
subject_data <- read_csv("../data/exp_probes/parsed_trials.csv") %>%
  select(-c(WID)) %>%
...
dbht_function <- function(data, indices) {
  d = data[indices,] # sample trials with replacement 
  ac_r <- cor.test(~ hr + a3_centroid, data = d)
  tc_r <- cor.test(~ hr + geo_centroid, data = d)
  nd_r <- cor.test(~ hr + dist_to_nd, data = d)
  or_r <- cor.test(~ hr + dist_to_center, data = d)
...
td_by_scene_tracker <- td_by_subj_tracker %>%
...
# human and model average performance
td_by_scene %>%
  summarise(td_human = mean(td_human),
            td_model = mean(td_model))
