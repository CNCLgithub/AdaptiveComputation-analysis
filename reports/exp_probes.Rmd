...
# Model design:
# scene, frame
probe_timings <- read_csv("../data/exp_probes/random_probe_timings.csv") %>%
...
exp_data <- read_csv("../data/exp_probes/model_probe_covariates.csv")

model_perf <- read_csv("../data/exp_probes/ac_td_perf.csv") %>%
  group_by(scene) %>%
...
ate_by_probe %>%
  ggplot(aes(x = a3_centroid_tot, y = hr)) +
  geom_point(size = 3.1,
             color = "#5aa67b",
           ) +
  geom_text(aes(label = ifelse(scene == scene_select, paste(scene, frame, tracker), ""))) +
  geom_smooth(method = "lm", color = "black") +
  theme_classic() +
  ggtitle("HR ~ Global Attention Centroid")
