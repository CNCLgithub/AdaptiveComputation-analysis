...
# model_perf <- read_csv("../data/exp2/11_11_2022_exp2_probes_target_designation_perf.csv") %>%
...
# model_att <- read_csv("../data/exp2/11_11_2022_exp2_probes_target_designation_att.csv")
...
inf_data <- model_att %>%
  ...
  unnest(cols = c(data, importance_smoothed, pred_x_smoothed, pred_y_smoothed)) %>%
  dplyr::select(-c(att_xy, pred_xs, pred_ys)) %>%
  ungroup()
...
centroids <- inf_data %>%
  ...
  summarise(total_cycles = sum(cycles),
            ...
            geo_x = mean(pred_x_smoothed),
            geo_y = mean(pred_y_smoothed)) %>%
  ungroup()
