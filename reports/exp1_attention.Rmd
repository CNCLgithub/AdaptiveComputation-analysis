...
model_perf <- read_csv("../data/exp2/exp2_probes_adaptive_computation_eu_perf.csv") %>%
  group_by(scene) %>%
...
leg_ate_by_probe <- read_csv("~/project/data/exp2/10_2022_ate-by-probe.csv") %>%
  select(scene, frame, a3_centroid_mu, importance_smoothed_mu) %>%
  rename(leg_a3_centroid = a3_centroid_mu,
         leg_importance_smoothed = importance_smoothed_mu)


ate_by_probe %>%
  left_join(leg_ate_by_probe) %>%
  ggplot(aes(x = importance_smoothed_mu, y = leg_importance_smoothed,
             label = paste(scene, frame, tracker))) +
  geom_point() + 
  geom_text(hjust=0, vjust=0)

ate_by_probe %>%
  left_join(leg_ate_by_probe) %>%
  ggplot(aes(x = paste(scene, frame, tracker), y = a3_centroid_mu - leg_a3_centroid,
             label = paste(scene, frame, tracker))) +
  geom_point() + 
  geom_text(hjust=0, vjust=0)
...
ate_by_probe %>%
  ggplot(aes(y = hr, x = cycles_smoothed_mu,
             label = paste(scene, frame, tracker))) +
  geom_point(size = 3.1,
             # color = "#5aa67b", 
             ) + 
...
summary()
# 
# print("Raw sensitivity")
# ate_by_probe %>%
#   with(lm(hr ~ sensitivity_smoothed_mu,)) %>%
#   summary()
...
<!-- leave_one_out_test <- function(data, indices) { -->

<!--   d <- data.frame(data) -->

<!--   nr = length(indices) -->

<!--   all_but_one = indices[2:nr] -->

<!--   the_one = indices[1:1] -->

<!--   group_a <- group_perf(d[all_but_one,], "a") -->

<!--   group_b <- group_perf(d[the_one,], "b") -->
...
<!-- loo_probe_hr_ci <- boot.ci(reps, type="bca") -->

<!-- loo_probe_hr_ci$mu <- mean(reps$t) -->
...
fig2di_df <- data.frame(
  ...
  yintercept = mean(shc_probe_hr_ci$mu))
