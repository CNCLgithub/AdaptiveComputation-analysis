...
model_att <- read_csv("../data/exp2/exp2_probes_target_designation_att.csv") %>%
  ...
  ungroup() %>%
  mutate(cycles_mu = cycles_mu + 1, # padding to allow for log
...
#   group_by(scene, frame) %>%
#   summarise(total_att = sum(att_smoothed),
#             total_exp_att = sum(att_smoothed / tau),
#             weighted_x = sum(pred_x_mu * (att_smoothed / tau) / total_exp_att),
#             weighted_y = sum(pred_y_mu * (att_smoothed / tau) / total_exp_att)) %>%
#   ungroup()
...
subject_performance <- td_by_subj %>%
  ...
  mutate(passed = (td_acc_mu - 0.5) > perf_thresh*td_acc_se & hr - fp > 0.1,
         high_perf = hr - fp > 0.4)
