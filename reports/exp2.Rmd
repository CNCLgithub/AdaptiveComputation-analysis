...
centroids <- inf_data %>%
  group_by(scene, frame) %>%
  summarise(total_cycles = sum(cycles_mu),
            weighted_x = sum(pred_x_mu * importance_smoothed),
            weighted_y = sum(pred_y_mu * importance_smoothed)) %>%
  ungroup()
...
timing_data <- good_subjects_data %>%
  ...
  select(-c(data, idata)) %>%
  unnest(cols = full) %>%
  select(scene, frame, tracker, ttwm, cycles_mu, probe, ID, response_frame) %>%
  ungroup()
...
wtm_fit <- ate_by_probe %>%
  with(lm(hr ~ ttwm + ttwm_mu))
wtm_fit %>%
  summary()

ate_by_probe$ttwm_resid = wtm_fit$residuals

ate_by_probe %>%
  ggplot(aes(x = pred_x_sd + pred_y_sd, y = abs(ttwm_resid))) +
  geom_point()
